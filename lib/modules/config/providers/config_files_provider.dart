...
final configFilesInitialProvider = Provider<ConfigFilesSettings>((_) => ConfigFilesSettings.defaults());

final configFilesProvider = NotifierProvider<ConfigFilesNotifier, ConfigFilesSettings>(
...
class ConfigFilesNotifier extends Notifier<ConfigFilesSettings> {
  ...
  Future<void> saveToDb(ConfigFilesSettings settings) async {
    ...
    await db.setSetting('auto_restart_on_crash', settings.autoRestartOnCrash ? '1' : '0');
    ...
    await db.setSetting('xms', settings.ramMinGb.isEmpty ? '' : '${settings.ramMinGb}G');
    await db.setSetting('xmx', settings.ramMaxGb.isEmpty ? '' : '${settings.ramMaxGb}G');
