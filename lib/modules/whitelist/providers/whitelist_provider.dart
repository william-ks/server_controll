...
class WhitelistState {
  ...
  factory WhitelistState.initial() =>
      const WhitelistState(players: [], loading: false);
}

final whitelistRepositoryProvider = Provider<WhitelistRepository>(
  (_) => WhitelistRepository(),
);
...
final whitelistProvider = NotifierProvider<WhitelistNotifier, WhitelistState>(
...
final whitelistRequirementsProvider = FutureProvider<WhitelistRequirements>((
  ref,
) async {
  ...
  final whitelistPath =
      await AppDatabase.instance.getSetting('whitelist_path') ??
      'whitelist.json';
  final fullPath = config.serverPath.trim().isEmpty
      ? whitelistPath
      : p.join(config.serverPath.trim(), whitelistPath);
  final hasWhitelistFile =
      config.serverPath.trim().isNotEmpty && File(fullPath).existsSync();
...
class WhitelistNotifier extends Notifier<WhitelistState> {
  ...
  Future<void> load({bool initialLoad = false}) async {
    ...
    try {
      ...
      await ref
          .read(playerPermissionsProvider.notifier)
          .syncWhitelist(normalized.map((item) => item.nickname).toList());
    } catch (error) {
  ...
  Future<void> refreshAndSyncFromFile() async {
    ...
    try {
      ...
      await ref
          .read(playerPermissionsProvider.notifier)
          .syncWhitelist(players.map((item) => item.nickname).toList());
    } catch (error) {
      state = state.copyWith(loading: false, error: error.toString());
    }
  }
