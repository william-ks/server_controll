...
class WhitelistSyncService {
  ...
  Future<List<WhitelistPlayer>> syncFromServerFile() async {
    final serverDir =
        await AppDatabase.instance.getSetting('server_dir') ??
    ...
    for (final player in refreshed) {
      ...
      final hasUuidInServerFile =
          uuid != null &&
          uuid.isNotEmpty &&
          serverUuids.contains(uuid.toLowerCase());
      final hasNameInServerFile = serverNames.contains(
  ...
  Future<void> syncPendingToServer({
    ...
    for (final player in pending) {
      await sendCommand(_commands.whitelistAdd(player.nickname));
      await _repository.upsertByNickname(
        player.copyWith(
          isPending: false,
          isAdded: true,
  ...
  Future<String?> persistIcon({
    ...
    final normalized = nickname.trim().replaceAll(
      RegExp(r'[^a-zA-Z0-9_\-]'),
      '_',
    );
    final safeName = normalized.isEmpty
